...
TEST_F(TestUnaryOperators, SumOperatorForward) {
  ...
  sum_op = SumOperator<Float>(x, {0});

  result = sum_op.forward();
  EXPECT_EQ(3, result.size());
  EXPECT_EQ(FloatTensorShape({3, 1}), result.shape());
  EXPECT_TRUE(xt::all(xt::equal(FloatTensor({5, 7, 9}), result)));

  // Sum over the second dimension.
  sum_op = SumOperator<Float>(x, {1});
